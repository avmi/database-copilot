...
export class MariaDBAdapter extends BaseAdapter {
  ...
  async getIndexes(table, schema) {
    const targetSchema = schema || this.config.database;
    const rows = await this._query(
      `SELECT INDEX_NAME, TABLE_NAME, GROUP_CONCAT(COLUMN_NAME ORDER BY SEQ_IN_INDEX) as columns_str,
    ...
    return rows.map(r => ({
      name: r.INDEX_NAME, tableName: r.TABLE_NAME, columns: r.columns_str.split(','),
  ...
  async executeQuery(query, limit = 100) {
    ...
    const rows = await this._query(limitedSql);
  ...
  async getSampleData(table, schema, limit = 10) {
    ...
    const start = Date.now();
    const rows = await this._query(`SELECT * FROM ${quotedSchema}.${quotedTable} LIMIT ${effectiveLimit}`);
    const columns = rows.length > 0 ? Object.keys(rows[0]) : [];
    return { columns, rows, rowCount: rows.length, executionTimeMs: Date.now() - start };
  }
